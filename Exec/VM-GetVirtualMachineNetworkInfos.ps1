...
try {
    ...
    if(-not($VMs)) { $VMs = (get-cluster |where-object{$_.Name -Match "wup"}|Get-VM)}
    if($VMs -is [system.String]) { $VMs = (Get-VM "$VMs")}
    $VMs = $($VMs| where-object {$_.pstypenames -contains "VMware.VimAutomation.ViCore.Impl.V1.Inventory.InventoryItemImpl"})
