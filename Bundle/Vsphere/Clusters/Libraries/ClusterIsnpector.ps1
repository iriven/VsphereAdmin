...
class PSIrivenCLSInfos{
    ...
    PSIrivenCLSInfos([VMware.VimAutomation.ViCore.Impl.V1.Inventory.ClusterImpl[]]$CLs,[HashTable]$Config)
    {
        try {
            if (-not([PSIrivenUtils]::PropertyExists($Config,'DisplayMode')) -or ($Config.DisplayMode.get_Count() -eq 0)) 
            {
                throw "Error: Can't initialize PSIrivenCLSInfos Class; Invalid Configuration Data Given."
            }
